...
lineReader.on('line', function (line) {
  if (line.includes(COMMAND)) {
    ...
    if (line.substring(line.length - 1) === ',') {
      iCommandContents = line.substring(line.length - 2) + RUN_COMMAND + ','; // with trailing comma
    } else {
      iCommandContents = line.substring(line.length - 1) + RUN_COMMAND; // without trailing comma
    }
    ...
    data.splice(iFoundLine, 0, iCommandContents); // write command line
